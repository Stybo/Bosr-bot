...
module.exports.run = async (bot, message, args) => {
    ...
    if (!message.member.hasPermission("BAN_MEMBERS")) return message.channel.send(embed1).then(msg => msg.delete({ timeout: 5000 }));

    if (!message.guild.me.hasPermission("BAN_MEMBERS")) return message.channel.send(embed1).then(msg => msg.delete({ timeout: 5000 }));
    ...
    if (!banUser) return message.channel.send(embed4).then(msg => msg.delete({ timeout: 5000 }));

    if (banUser.hasPermission("BAN_MEMBERS")) return message.channel.send(embed5).then(msg => msg.delete({ timeout: 5000 }));
    ...
    message.channel.send(embedPrompt).then(async msg => {
        ...
        if (emoji === "✅") {
            ...
            message.reply("Ban geanuleerd").then(msg => msg.delete({ timeout: 3000 }));
